        ...
        <div className="container">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-text-muted">
              <strong className="text-foreground">ข้อมูลสำคัญ:</strong> ข้อมูลในหน้านี้เป็นข้อมูลทั่วไปเพื่อประกอบการพิจารณาเท่านั้น ไม่ถือเป็นคำแนะนำด้านการลงทุนหรือภาษี ผลตอบแทนที่แสดงเป็นการประมาณการ กรุณาปรึกษาที่ปรึกษาทางการเงินก่อนตัดสินใจ
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 lg:py-20 bg-background">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="mb-8">
            ...
            <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-2">รูปแบบการลงทุน</h2>
            <p className="text-sm text-text-secondary max-w-2xl">เลือกรูปแบบที่เหมาะกับสถานะทางการเงินและเป้าหมายของธุรกิจคุณ</p>
          </motion.div>
            ...
            {investmentModels.map((model, i) => (
              <motion.div key={model.title} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-display text-base font-bold text-foreground">{model.title}</h3>
                  {model.highlight && (
                    <span className="px-2 py-0.5 text-[10px] font-medium bg-accent-primary text-text-inverse rounded-md ml-auto">แนะนำ</span>
                  )}
                </div>
                ...
                <div className="pt-3 border-t border-border-subtle">
                  <p className="text-[10px] text-text-muted"><strong className="text-foreground">เหมาะกับ:</strong> {model.suitable}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="max-w-3xl mx-auto">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs font-medium text-accent-secondary tracking-widest uppercase">Solar Carport ROI Example</span>
            </div>
            <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-5">ตัวอย่าง ROI: Solar Carport 100 คัน</h2>
            <div className="p-5 lg:p-6 rounded-xl border border-border-accent bg-surface-elevated">
              <h3 className="font-display font-semibold text-foreground text-sm mb-4">ห้างสรรพสินค้า / อาคารพาณิชย์ — ลานจอดรถ 100 คัน</h3>
              <div className="grid grid-cols-2 gap-3 mb-5">
                {[
                  ...
                  { label: "ผลตอบแทนตลอดอายุ 25 ปี", value: "40-70 ล้านบาท", accent: true },
                  { label: "รายได้เสริม EV Charging", value: "มีโอกาส" },
                ].map((item) => (
                  <div key={item.label} className="p-3 rounded-lg bg-accent-glow">
                    <div className="text-[10px] text-text-muted mb-0.5">{item.label}</div>
                    <div className={`font-display text-sm font-bold ${item.accent ? "text-accent-primary" : "text-foreground"}`}>{item.value}</div>
                  </div>
                ))}
              </div>
              <div className="flex items-start gap-2 p-3 rounded-lg bg-accent-secondary/10 border border-accent-secondary/20 mb-4">
                ...
                <p className="text-[10px] text-text-muted">
                  ตัวเลขข้างต้นเป็นการประมาณการเบื้องต้น ผลลัพธ์จริงขึ้นอยู่กับทำเลที่ตั้ง ปริมาณการใช้ไฟฟ้า อัตราค่าไฟ และขนาดระบบ
                </p>
              </div>
              <Link href="/solar-carport" className="inline-flex items-center gap-2 text-xs font-medium text-accent-primary hover:underline">
                ดูรายละเอียด Solar Carport <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-14 lg:py-20 bg-background">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="max-w-3xl mx-auto">
            ...
            <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-5">ตัวอย่าง ROI: Rooftop Solar 500 kW</h2>
            <div className="p-5 lg:p-6 rounded-xl border border-border-subtle bg-surface-elevated">
              <h3 className="font-display font-semibold text-foreground text-sm mb-4">โรงงานขนาดกลาง (ตัวอย่าง)</h3>
              <div className="grid grid-cols-2 gap-3 mb-5">
                {[
                  { label: "เงินลงทุนโดยประมาณ", value: "12-15 ล้านบาท" },
                  { label: "ประหยัดค่าไฟต่อปี", value: "3-4 ล้านบาท" },
                  { label: "คืนทุนโดยประมาณ", value: "3-5 ปี", accent: true },
                  { label: "ผลตอบแทนตลอดอายุ 25 ปี", value: "60-80 ล้านบาท", accent: true },
                ].map((item) => (
                  <div key={item.label} className="p-3 rounded-lg bg-accent-glow">
                    <div className="text-[10px] text-text-muted mb-0.5">{item.label}</div>
                    <div className={`font-display text-sm font-bold ${item.accent ? "text-accent-primary" : "text-foreground"}`}>{item.value}</div>
              ...
              <div className="flex items-start gap-2 p-3 rounded-lg bg-accent-secondary/10 border border-accent-secondary/20">
                <AlertTriangle className="w-3.5 h-3.5 text-accent-secondary shrink-0 mt-0.5" />
                <p className="text-[10px] text-text-muted">
                  ตัวเลขข้างต้นเป็นการประมาณการเบื้องต้น ผลลัพธ์จริงขึ้นอยู่กับหลายปัจจัย กรุณาติดต่อเราเพื่อรับการประเมินเฉพาะสำหรับธุรกิจของคุณ
                </p>
              </div>
            </div>
          </motion.div>
      ...
      <section className="py-14 lg:py-20 section-alt">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="mb-8">
            ...
            <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-2">สิทธิประโยชน์ทางภาษีที่เป็นไปได้</h2>
            <p className="text-sm text-text-secondary max-w-2xl">การลงทุนพลังงานสะอาดอาจได้รับสิทธิประโยชน์ทางภาษีหลายรูปแบบ ขึ้นอยู่กับเงื่อนไขของแต่ละโครงการ</p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {taxBenefits.map((tb, i) => (
              <motion.div key={tb.title} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}
                ...
                <h3 className="font-display font-semibold text-foreground text-sm mb-1.5">{tb.title}</h3>
                <p className="text-xs text-text-muted leading-relaxed">{tb.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="container">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-1.5 text-[10px] text-text-muted leading-relaxed">
              ...
              <p>3. สิทธิประโยชน์ทางภาษีอาจเปลี่ยนแปลงได้ตามนโยบายของรัฐบาล กรุณาตรวจสอบข้อมูลล่าสุดจากกรมสรรพากร</p>
      ...
      <section className="py-16 lg:py-24 section-alt relative">
        ...
        <div className="container relative text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}>
            <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-3">ต้องการประเมิน ROI เฉพาะธุรกิจคุณ?</h2>

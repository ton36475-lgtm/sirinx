...
import { ArrowLeft, ArrowRight, Clock, Tag, Share2, BookOpen, User } from "lucide-react";
...
const articleContent: Record<string, { sections: { heading: string; body: string }[]; keyTakeaways: string[] }> = {
  "rooftop-solar-roi-2025": {
    sections: [
      {
        heading: "ทำไม Rooftop Solar ถึงคุ้มค่าในปี 2025",
        body: "ราคาแผงโซลาร์ลดลงกว่า 90% ในช่วง 15 ปีที่ผ่านมา ขณะที่ค่าไฟฟ้าในประเทศไทยปรับตัวสูงขึ้นอย่างต่อเนื่อง ทำให้ระยะเวลาคืนทุนของ Rooftop Solar สั้นลงเหลือเพียง 3-5 ปี สำหรับโรงงานที่มีค่าไฟเกิน 100,000 บาทต่อเดือน\n\nปัจจัยสำคัญที่ทำให้ ROI ดีขึ้นในปี 2025 ได้แก่ ประสิทธิภาพแผง TOPCon ที่สูงถึง 22% ต้นทุน inverter ที่ลดลง และนโยบายสนับสนุนจากภาครัฐ เช่น การหักค่าเสื่อมราคาเร่ง"
      },
      {
    ...
    keyTakeaways: [
      "Rooftop Solar คืนทุนเฉลี่ย 3-5 ปี สำหรับโรงงานในไทย",
...
export default function BlogPost() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-display text-3xl font-bold text-foreground mb-4">ไม่พบบทความ</h1>
          <p className="text-text-secondary mb-6">บทความที่คุณค้นหาอาจถูกย้ายหรือลบแล้ว</p>
          <Link href="/blog" className="inline-flex items-center gap-2 px-6 py-3 btn-accent rounded-lg font-display font-semibold">
  ...
  const related = blogPosts.filter((p) => p.categoryKey === post.categoryKey && p.slug !== slug).slice(0, 3);
    ...
    <div>
      ...
      <section className="py-24 lg:py-32 bg-background">
        <div className="container max-w-4xl">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
            <Link href="/blog" className="inline-flex items-center gap-1.5 text-sm text-accent-primary hover:underline mb-6">
              <ArrowLeft className="w-4 h-4" /> กลับไปหน้าบทความ
            </Link>
          </motion.div>
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={1} className="flex items-center gap-3 mb-4">
            <span className="px-2.5 py-1 text-xs font-medium bg-accent-glow text-accent-primary rounded-md border border-border-accent">{post.category}</span>
            ...
            <article>
              {content ? (
                content.sections.map((section, i) => (
                  <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="mb-10">
                    <h2 className="font-display text-xl lg:text-2xl font-bold text-foreground mb-4">{section.heading}</h2>
                    {section.body.split("\n\n").map((para, j) => (
                      <p key={j} className="text-text-secondary leading-relaxed mb-4 whitespace-pre-line">{para}</p>
                    ))}
                  </motion.div>
                ))
              ) : (
                <div className="p-6 rounded-xl border border-border-subtle bg-surface-elevated">
                  <p className="text-text-secondary">เนื้อหาบทความนี้กำลังอยู่ระหว่างการจัดทำ กรุณากลับมาอ่านอีกครั้งในเร็ว ๆ นี้</p>
                </div>
              )}

              {/* CTA in article */}
              <div className="mt-12 p-6 rounded-xl glass-card">
                <h3 className="font-display font-semibold text-foreground mb-2">สนใจระบบโซลาร์สำหรับธุรกิจ?</h3>
                <p className="text-sm text-text-secondary mb-4">ใช้เครื่องมือคำนวณของ SIRINX เพื่อประเมินขนาดระบบและผลตอบแทนเบื้องต้น ฟรี</p>
                <div className="flex flex-col sm:flex-row gap-3">
                  <Link href="/assessment" className="inline-flex items-center justify-center gap-2 px-5 py-2.5 btn-accent rounded-lg text-sm font-display font-semibold">
                    คำนวณระบบโซลาร์ <ArrowRight className="w-4 h-4" />
                  </Link>
                  <Link href="/contact" className="inline-flex items-center justify-center gap-2 px-5 py-2.5 btn-accent-outline rounded-lg text-sm font-display font-semibold">
                    นัดสำรวจหน้างานฟรี
                  </Link>
                </div>
              </div>
              ...
              <div className="mt-8 flex items-center gap-2 flex-wrap">
                ...
                {post.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 text-xs rounded-full border border-border-subtle text-text-muted hover:border-border-accent hover:text-accent-primary transition-colors">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              {content?.keyTakeaways && (
                <div className="sticky top-24 p-5 rounded-xl border border-border-subtle bg-surface-elevated">
                  <h3 className="font-display font-semibold text-foreground mb-4 flex items-center gap-2">
                    <BookOpen className="w-4 h-4 text-accent-primary" /> สรุปประเด็นสำคัญ
                  </h3>
                  <ul className="space-y-3">
                    {content.keyTakeaways.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-text-secondary">
                        <span className="w-5 h-5 rounded-full bg-accent-glow text-accent-primary text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                        {item}
                      </li>
                    ))}
                  </ul>
      ...
      {related.length > 0 && (
        <section className="py-16 lg:py-20 section-alt">
          <div className="container">
            <h2 className="font-display text-xl font-bold text-foreground mb-8">บทความที่เกี่ยวข้อง</h2>
            <div className="grid md:grid-cols-3 gap-6">
              {related.map((rp, i) => (
                <motion.div key={rp.slug} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}>
                  <Link href={`/blog/${rp.slug}`} className="group block rounded-xl border border-border-subtle bg-surface-elevated overflow-hidden hover:border-border-accent transition-all h-full">
                    ...
                    <div className="p-4">
                      <span className="text-xs text-accent-secondary">{rp.category}</span>
                      <h3 className="font-display font-semibold text-foreground mt-1 mb-2 group-hover:text-accent-primary transition-colors text-sm">{rp.title}</h3>
      ...
      <section className="py-16 lg:py-20 bg-background">
        <div className="container text-center">
          <h2 className="font-display text-2xl lg:text-3xl font-bold text-foreground mb-3">พร้อมเริ่มต้นลดค่าพลังงาน?</h2>
          <p className="text-text-secondary mb-6 max-w-lg mx-auto">นัดสำรวจหน้างานฟรี ไม่มีข้อผูกมัด รับข้อเสนอที่ออกแบบเฉพาะสำหรับธุรกิจของคุณ</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact" className="inline-flex items-center justify-center gap-2 px-6 py-3 btn-accent rounded-lg font-display font-semibold">
              นัดสำรวจหน้างานฟรี <ArrowRight className="w-4 h-4" />
            </Link>
            <Link href="/assessment" className="inline-flex items-center justify-center gap-2 px-6 py-3 btn-accent-outline rounded-lg font-display font-semibold">

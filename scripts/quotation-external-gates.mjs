...
function envGroupStatus(group) {
  const configuredNames = group.names.filter(name => Boolean(process.env[name]));
  return {
    id: group.id,
    label: group.label,
...
const envChecks = envGroups.map(envGroupStatus);
const missingEnv = envChecks.filter(check => check.status !== "configured");
const databaseConfigured = hasAny(["DATABASE_URL"]);
const notificationConfigured =
  hasAny(["BUILT_IN_FORGE_API_URL", "FORGE_API_URL"]) &&
  hasAny(["BUILT_IN_FORGE_API_KEY", "FORGE_API_KEY"]);
const oauthConfigured =
  hasAny(["OAUTH_SERVER_URL"]) &&
  hasAny(["VITE_OAUTH_PORTAL_URL"]) &&
  hasAny(["VITE_APP_ID"]);
...
const report = {
  ...
  nextActions:
    ready
      ? [
          "Run pnpm quote:gate with the same target runtime env.",
          "Run pnpm quote:db:preflight before pnpm db:push.",
          "Deploy through the approved Cloudflare path and run one approved smoke quotation.",
        ]
      : [
          ...missingEnv.map(
